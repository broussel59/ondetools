...
#' Génère le graphique et le tableau des périodes de retour de l'indice ONDE avec bandes pleines
#'
#' @param onde_df Dataframe contenant les données brutes téléchargées via l'API ONDE.
#' @param code_dpt Code du département (ex: "59").
#' @param annee_courante Année de suivi à afficher en courbe (par défaut 2026).
#' @param force_complementaire Inclure les campagnes complémentaires (FALSE par défaut).
#' @param dossier_sortie Chemin du dossier d'export des résultats (NULL si pas d'export).
#'
...
produire_periodes_retour_indice_onde_courbes_2026 <- function(onde_df,
  ...
  # 3. Calcul des quantiles (PERCENTILE.INCLURE Excel)
  ...
  # 4. Construction du graphique avec rubans (geom_ribbon)
  y_min <- max(0, floor(min(c(stats_retours_wide$Record, data_annee_courante$indice), na.rm = TRUE)))
  
  p <- ggplot2::ggplot(data = stats_retours_wide, ggplot2::aes(x = Mois)) +
    # Rubans colorés
    ggplot2::geom_ribbon(
      ggplot2::aes(ymin = Record, ymax = `20 ans`, group = 1, fill = "Record à 20 ans"),
      alpha = 0.6
    ) +
    ggplot2::geom_ribbon(
      ggplot2::aes(ymin = `20 ans`, ymax = `10 ans`, group = 1, fill = "20 ans à 10 ans"),
      alpha = 0.6
    ) +
    ggplot2::geom_ribbon(
      ggplot2::aes(ymin = `10 ans`, ymax = `5 ans`, group = 1, fill = "10 ans à 5 ans"),
      alpha = 0.6
    ) +
    ggplot2::geom_ribbon(
      ggplot2::aes(ymin = `5 ans`, ymax = `3 ans`, group = 1, fill = "5 ans à 3 ans"),
      alpha = 0.6
    ) +
    ggplot2::geom_ribbon(
    ...
    # Échelles et palettes
    ggplot2::scale_fill_manual(
      name = NULL,
      values = c(
        "Record à 20 ans"             = "#d73027",
        "20 ans à 10 ans"             = "#fdae61",
        "10 ans à 5 ans"              = "#fee08b",
        "5 ans à 3 ans"               = "#a6d96a",
        "> 3 ans (Situation usuelle)" = "#4575b4"
      ),
      breaks = c("Record à 20 ans", "20 ans à 10 ans", "10 ans à 5 ans", "5 ans à 3 ans", "> 3 ans (Situation usuelle)")
    ) +
  ...
  # 5. Export des fichiers si un dossier est renseigné
  if (!is.null(dossier_sortie)) {
    dir.create(dossier_sortie, showWarnings = FALSE, recursive = TRUE)
    
    tableau_large <- tableau_annuel %>%
      tidyr::pivot_wider(names_from = Mois, values_from = indice)
    
    readr::write_csv(tableau_large, file.path(dossier_sortie, paste0("indice_onde_", min(annees_reference), "_", annee_courante, "_", code_dpt, ".csv")))
    readr::write_csv(tableau_retours, file.path(dossier_sortie, paste0("periodes_retour_onde_", code_dpt, ".csv")))
